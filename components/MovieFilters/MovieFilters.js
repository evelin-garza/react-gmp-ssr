...
const MovieFilters = (props) => {
    ...
    return (
        <div className={`${styles['movie-filters']} container`}>
            <div className={styles.categories}>
                {filters.length &&
                    filters.map((filter, index) => (
                        <button key={index} onClick={onGenreSelect} className={(filter.toLowerCase() == genreFilter) ? styles.active : ''}>{filter}</button>
                    ))}
            </div>
            <div className={styles.sorting}>
                <span>SORT BY </span>
                <div className={styles['select-sort-container']}>
